...
export default function ${featurePascal}DetailsForm<E, S extends string>({
  ...
  const { ${featureCamel}FormTranslations } = use${featurePascal}Translations();

  return (
    <CustomForm
      form={form}
      getLabel={${featureCamel}FormTranslations("fields")}
...
function columnsTemplate(featurePascal: string) {
  return `// Columns.tsx
"use client";
import type { ${featurePascal}List200Data } from "@/src/api/model";
...
export const ${featurePascal}Columns = ({ getHeader }: { getHeader: any }) =>
  createColumns<${featurePascal}List200Data["list"][number]>({
  ...
  return (
    <DataTable
      columns={${featurePascal}Columns({
        getHeader: ${featureCamel}ColumnHeaderTranslations as (key: keyof ${featurePascal}List200Data["list"][number]) => string,
      })}
      data={data.data.list}
      isLoading={isPending}
    />
